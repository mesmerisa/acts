...
std::shared_ptr<const Acts::TrackingGeometry> buildTGeoBGVDetector(
  ...
  //std::cout << "Cone dims: "<< shape2->GetDz()  << " "  <<  shape2->GetRmax1() << " " << shape2->GetRmin1() << " " <<  shape2->GetRmax2() << " " << shape2->GetRmin2() << std::endl;  
  ...
  std::cout << "Bound box shape of LRBP: "<< LRBPShape->GetDX() << " " <<  LRBPShape->GetDY() << " " << LRBPShape->GetDZ() << " " << std::endl;
  ...
  double lower = 2*EWConeShape->GetDz()*10; // length of cone, cm to mm
  double ConeOpeningAng = 2.0 * atan(upper/lower);
  
  //auto coneBounds = std::make_shared<Acts::ConeBounds>(ConeOpeningAng, zMaxInteraction, zMaxTransition, M_PI, 0.0);
  double LenCutOffTip = rBeamPipe/(tan(ConeOpeningAng*0.5));
  auto coneBounds = std::make_shared<Acts::ConeBounds>(ConeOpeningAng, LenCutOffTip, 2.0*EWHalfLength, M_PI, 0.0);
  ...
  auto interactionVolume = 
   ...
   // (b) transition volume - @TODO layers to be added later
   //auto transitionVolume = cylinderVolumeHelper->createTrackingVolume(geoCtx, mtvVector, nullptr, 0., zMaxInteraction, zMaxTransition, "Transition");
   // (c) beampipe volume
   auto bpVolume = 
   ...
   // (d) BGV volume - @TODO layers to be added later
   auto bgvVolume = cylinderVolumeHelper->createTrackingVolume(geoCtx, emptylayarr, mtvVector, nullptr, rBeamPipe, rMax, zMaxTransition, zMaxBeamPipe, "BGV");

   // TrackingGeometry magic, part a:
   ...
   // TrackingGeometry magic, part b:
   // now we create a world volume containing interactionVolume / transitionVolume/ bgBgvContainer
   ...
   return std::make_shared<Acts::TrackingGeometry>(worldContainer); //, mdecorator); // XXX
