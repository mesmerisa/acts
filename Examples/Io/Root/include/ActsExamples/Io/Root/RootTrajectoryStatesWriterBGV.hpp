...
/// @class RootTrajectoryStatesWriterBGV
///
...
///
/// Safe to use from multiple writer threads - uses a std::mutex lock.
class RootTrajectoryStatesWriterBGV final : public WriterT<TrajectoriesContainer> {
 public:
  struct Config {
    ...
    /// name of the output tree for wrong tracks.
    std::string outputTreename_wrong = "tracks_wrong";
    /// file access mode.
  ...
  RootTrajectoryStatesWriterBGV(const Config& cfg, Acts::Logging::Level lvl);
  ~RootTrajectoryStatesWriterBGV() final override;
 ...
 private:
  ...
  // XXX
  TTree* m_outputTree_wrong{nullptr};  ///< The output tree
  int m_eventNr_wrong{0};
  int m_trajNr_wrong{0};  
  int true_or_not_wrong{0};
  int  true_or_not{0}; 
  std::vector<float> m_chi2_wrong; 
  // XXX
