...
///
/// Safe to use from multiple writer threads - uses a std::mutex lock.
class RootTrajectoryWriterBGV final : public WriterT<TrajectoriesContainer> {
 public:
  struct Config {
  ...
  RootTrajectoryWriterBGV(const Config& cfg, Acts::Logging::Level lvl);
  ~RootTrajectoryWriterBGV() final override;
