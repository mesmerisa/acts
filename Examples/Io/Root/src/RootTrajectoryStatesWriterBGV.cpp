...
ActsExamples::RootTrajectoryStatesWriterBGV::RootTrajectoryStatesWriterBGV(
    const ActsExamples::RootTrajectoryStatesWriterBGV::Config& cfg,
    Acts::Logging::Level lvl)
    : WriterT(cfg.inputTrajectories, "RootTrajectoryStatesWriterBGV", lvl),
      m_cfg(cfg),
  ...
  if (m_outputTree == nullptr)
   ...
   // I/O parameters for "wrong" tree  XXX
...
ActsExamples::ProcessCode ActsExamples::RootTrajectoryStatesWriterBGV::endRun() {
  if (m_outputFile) {
    m_outputFile->cd();
    m_outputTree->Write();
...
ActsExamples::ProcessCode ActsExamples::RootTrajectoryStatesWriterBGV::writeT(
  ...
  for (size_t itraj = 0; itraj < trajectories.size(); ++itraj) {
    ...
    for (unsigned int isubtraj = 0; isubtraj < trackTips.size(); ++isubtraj) {
      ...
      if (particleHitCounts.size() == 1 &&  std::find(selectedParticles.begin(), selectedParticles.end(), particleHitCounts.front().particleId.value()) != selectedParticles.end() ) { //&& inTruthParticleColl) { // if only 1 particle contributes - these are the true tracks.                              
        std::cout << "particleHitCounts.front().particleId.value() " << particleHitCounts.front().particleId.value() << std::endl; 
        true_or_not = 1;
        ...
        if (ip != particles.end()) {
          ...
          ACTS_WARNING("Truth particle with barcode = " << barcode
            ...
            //std::cout << "else ........... particleHitCounts.front().particleId.value() " << particleHitCounts.front().particleId.value() << std::endl; 
            true_or_not = 0;
            counter_else ++;      
