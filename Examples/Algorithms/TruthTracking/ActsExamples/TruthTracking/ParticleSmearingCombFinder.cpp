...
ActsExamples::ParticleSmearingCombFinder::ParticleSmearingCombFinder(const Config& cfg,
                                                 Acts::Logging::Level lvl)
    : BareAlgorithm("ParticleSmearingCombFinder", lvl), m_cfg(cfg) {
...
ActsExamples::ProcessCode ActsExamples::ParticleSmearingCombFinder::execute(
  ...
  for (auto cand : trackCandidates) {
     ...
     for (auto sh: simHits) {
         if(counter == first_hit_ind) {
              ...
              //std::cout << "Id " << sh.particleId() << std::endl;
              //if (std::find(particles_ids.begin(), particles_ids.end(), sh.particleId().subParticle() ) != particles_ids.end() ) {
...
//  for (auto&& [vtxId, vtxParticles] : groupBySecondaryVertex(particles)) {
    ...
    for (std::size_t n = 0; n < std::min( first_hit_pos.size(), last_hit_pos.size() ); n++) {
       ...
       double p = 0.2797*exp(atanh(vec_cand[2]/r) - 0.6819);
       //double px = p*vec_cand[0]/r;
       //double py = p*vec_cand[1]/r;
       double pt = p*sin(theta); //sqrt(px*px + py*py);

       //std::cout << "p ------------------- " << p << " pt ----------- " << pt << std::endl; 
    
       //double p = 4; //3;  // same as above 
       double q = 1; //1;
       ////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //std::cout << "len of paras: " << parameters.size() << std::endl;
   ...
   /* for (auto&& [vtxId, vtxParticles] : groupBySecondaryVertex(particles)) {
      for (const auto& particle : vtxParticles) {
          ...
          const auto p2 = particle.absoluteMomentum();
          const auto q2 = particle.charge();
          
          //std::cout << "true --------    phi, theta "  << " " << phi2 << " " << theta2 << std::endl;
          //std::cout << "true --------    p, pt "  << " " << p2 << " " << pt2 << std::endl;
      }
  }*/
      //std::cout << "-----------------------------------------------------------------" << std::endl;
